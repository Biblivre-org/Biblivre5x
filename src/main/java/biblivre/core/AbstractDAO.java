...
public abstract class AbstractDAO {
	...
	public final void fixSequence(String sequence, String tableName, String columnName) {
		Connection con = null;

		try {
			con = this.getConnection();

			String sql = "SELECT setval('" + sequence + "', coalesce((SELECT max(" + columnName + ") + 1 FROM " + tableName + "), 1), false);";
			PreparedStatement pst = con.prepareStatement(sql);
